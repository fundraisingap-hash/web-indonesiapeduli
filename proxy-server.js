// proxy-server.js — Xenith Payment Gateway Proxy
    ...
    req.on('end', () => {
      try {
        ...
        if (!apiKey || !secretKey) {
          ...
          res.end(JSON.stringify({ error: true, message: 'API Key & Secret Key wajib diisi di Pengaturan Admin → Payment Gateway → Xenith.' }));
          return;
        }
